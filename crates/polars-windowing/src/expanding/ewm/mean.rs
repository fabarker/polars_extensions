use polars_arrow::legacy::kernels::ewm::ewm_mean as kernel_ewm_mean;
...
pub fn ewm_mean(
    ...
    match s.dtype() {
        DataType::Float32 => {
            let xs = s.f32().unwrap();
            let result = kernel_ewm_mean(xs, alpha as f32, adjust, min_periods, ignore_nulls);
            Series::try_from((s.name().clone(), Box::new(result) as ArrayRef))
        },
        DataType::Float64 => {
            let xs = s.f64().unwrap();
            let result = kernel_ewm_mean(xs, alpha, adjust, min_periods, ignore_nulls);
            Series::try_from((s.name().clone(), Box::new(result) as ArrayRef))
        },
        _ => ewm_mean(
