...
class ExponentialMoving(Window):
    ...
    def __init__(self, expr: pl.Expr, window_type: WinType):
    ...
    def __call__(self, **kwargs) -> Self:
        if (bias := kwargs.get("bias")) is not None and isinstance(bias, bool):
            self._bias = kwargs.pop("bias")

        if (bias := kwargs.get("adjust")) is not None and isinstance(bias, bool):
